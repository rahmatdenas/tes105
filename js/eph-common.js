...
Semua placeholder Anda yang sudah ada (JENIS, KURUNG_BUKA/TUTUP, WILAYAH_1, PROP_LOKASI, HIERARKI_LOKASI, UNION_EKSTRA, PROP_TAHUN) posisinya tetap sama persis, tidak perlu ubah kode yang membangun clause-clause itu di populateProvinceTypesData. Hanya satu placeholder baru ditambahkan: <PLACEHOLDER_LIMIT_OFFSET>, sengaja diletakkan di dalam subquery (bukan diisi lewat .replace() sekali di awal, tapi diisi ulang tiap halaman — lihat poin 3).

Catatan: saya lihat kode Anda juga punya .replace(/<PLACEHOLDER_FILTER_NASIONAL>/g, filterNasional), tapi token <PLACEHOLDER_FILTER_NASIONAL> tidak ada di template query manapun yang Anda kirim. Ini kemungkinan sisa kode lama — tidak berbahaya (regex replace tanpa match = tidak melakukan apa-apa), tapi kalau memang filterNasional seharusnya dipakai di suatu tempat (misal untuk klaster nasional), beri tahu saya di mana seharusnya token itu berada di query.

2. Fungsi baru — tambahkan di JS 1 (dekat queryWdqsThenProcess)
javascript
// FUNGSI BARU #1: Fetch mentah satu halaman, parsing JSON dibungkus try/catch
...
async function queryWdqsPaginated(queryTemplate, processEachResult, postprocessCallback, chunkSize = 5000) {
  ...
  while (true) {
    ...
    // KUNCI PERBAIKAN: hitung entitas UNIK di halaman ini, bukan jumlah baris mentah
    let entitasUnik = new Set(bindings.map(b => b.SQ.value)).size;
    if (entitasUnik < chunkSize) break; // halaman terakhir
